...
게임 설정 막대기 중 다음은 선수 인원이다. 선수 몇 명이 참가하는지와 기기 몇 대가 사용되는지는 독립적이라서 선수 인원이 기기 목록에서 분리된다. 선수 인원이 현재 **경쟁** 차례 모드에만은 작용된다.
...
차례 모드는 선수가 각각의 차례를 기다리든지 (**경쟁**) 모두 언제나 동시에 행동하든지 (**공동**)를 수정한다.

**경쟁** 경우는, 현 차례의 선수 번호가 점수 옆에 분수 분자로 표시되고, 남는 선수 인원이 분모로 보인다. 한 차례의 총 명령 수량과 현 명령 번호는 둘 다 동적이라서 진행 표시줄로 보인다. 그런데도 점수가 선수끼리 공용적이지만, 자신의 차례에서 경기 판을 마치는 누군가는 탈락된다.
...
차례 모드가 **경쟁**이면은, 게임 설정 막대기 끝머리에 가까운 재시작 버튼을 누르셔서 1판으로 돌아가고 탈락 인원이 다시 0명이겠다.
